...
class TRDefender(Tactic):
    def __init__(self,bot_id,state,params=None):
        super(TRDefender, self).__init__(bot_id, state, params)
    ...
    def execute(self, state , pub):
        ...
        if gameState == TRDefender.State.block:
            ...
            self.sParams.GoToPointP.finalSlope = ballPos.normalizeAngle(finalPos.angle(ballPos))
        ...
        elif gameState == TRDefender.State.intercept:
            ...
            self.sParams.GoToPointP.y = y
            finalPos = Vector2D(self.sParams.GoToPointP.x,self.sParams.GoToPointP.y)
            self.sParams.GoToPointP.finalSlope = ballPos.normalizeAngle(ballPos.angle(finalPos))
            if distance <= DRIBBLER_BALL_THRESH and ballPos.y > 0:
                ...
                sGoToPoint.execute(self.sParams, state, self.bot_id, pub)
        elif gameState == TRDefender.State.chill :
            ...
            print (pointL.x,",",pointL.y)
            print (pointR.x,",",pointR.y)
            print ("_________________________________")
            self.sParams.GoToPointP.x = pointR.x
            self.sParams.GoToPointP.y = pointR.y
            finalPos = pointR
            ...
            if distance <= DRIBBLER_BALL_THRESH and ballPos.y < 0:
                ...
                if finalPos.dist(state.homePos[self.bot_id]) <= BOT_BALL_THRESH :
                    ...
                    print (" GET POSITIONED ")
                    sGoToPoint.execute(self.sParams, state, self.bot_id, pub)
        elif gameState == TRDefender.State.clear_block:

...
void CallBack(const krssg_ssl_msgs::gr_Commands::ConstPtr& msg)
{
    ...
    if(msg->robot_commands.id == 0){
        ...
        // write the data for first 3 bots only **16 bytes**
        int _start = 1+msg->robot_commands.id*5, c=0;
        for(int i=_start;i<_start + 4;i++, c++){

            if((int)vel_wheel[c]!=127)
                buf[i] = (int)vel_wheel[c];
            else
                buf[i]=126;
        }

        if (msg->robot_commands.kickspeedx > 6 && msg->robot_commands.spinner)
        {
            buf[_start+4] = 3;
        }
        else if(msg->robot_commands.kickspeedx > 6)
        {
            
            buf[_start+4] = 1;
        }
        else if(msg->robot_commands.spinner )
        {
            buf[_start + 4] = 2;
        }
        else
...
int main(int argc, char *argv[])
{
    ...
    ros::init(argc, argv, "bot_comm_normal");
    ros::NodeHandle n;

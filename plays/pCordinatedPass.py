...
class pCordinatedPass(object):
    ...
    def tactic_instance(self, bot_id, tactic_id, params, other_bot_id=-1):
        ...
        if tactic_id == "TAttacker":
            instance = tactics.TAttacker.TAttacker(0, self.state, params)
        elif tactic_id == "TReceiver":
            instance = tactics.TReceiver.TReceiver(bot_id, self.state, params)
        else:
            instance=None
        return instance
    ...
    def bs_callback(self, data):
        ...
        tac_1 = self.tactic_instance(0, self.role_list[0][0], self.role_list[0][1])     #attacker
        tac_1.updateParams(self.state)
        tac_1.execute(self.state, self.pub, 1)

        tac_2 = self.tactic_instance(1, self.role_list[1][0], self.role_list[1][1])     #receiver
        tac_2.updateParams(self.state)
        tac_2.execute(self.state, self.pub, 0)
